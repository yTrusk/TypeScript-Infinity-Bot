...
import { EmbedCreator, createGuild, errorreport, handle } from "../../functions/functions";
...
export default new Event({
  ...
  async run(guild) {
    ...
    const embed = await EmbedCreator({title: `<a:planeta:1084627835408363640> | ${guild.name} (${guild.id})`, description: `<:tabela:1084631840528281701> **Fui adicionado no servidor:** \`${guild.name}\` \n<:cliente:1084634375997632582> **Membros:** \`${guild.memberCount}.\` \n<:info:1084952883818143815> **Totalizando** \`${client.guilds.cache.size}\` **servidores e** \`${client.users.cache.size}\` **usuários.**`})

    ...
    if (q < 10) {
      ...
      if (bal < q) {
        ...
        if (ultimoCmd !== null && timeout - (Date.now() - ultimoCmd) > 0) {
          ...
          const embed_err = await EmbedCreator({title: `<a:errado:1084631043757310043> Erro, double em cooldown!`, description: `Vagabundo querendo trapacear né? Espera \`${time}s\` para usar o double novamente!`})
          interaction.reply({ embeds: [embed_err], ephemeral: true });
          return;
        } else {
          cooldowns[userid.id].lastCmd = Date.now();
        }
        const embed1s = await EmbedCreator({description: `<a:carregando:1084633391820980254> **Realizando aposta...**`})
        ...
        if (quantia < 6) {
          ...
          const embedlose = await EmbedCreator({title: `<a:errado:1084631043757310043> Lose`, description: `**Você perdeu** \`${q}\``})
          ...
          const embedwin = await EmbedCreator({title: `<a:certo:1084630932885078036> Win`, description: `**Você ganhou** \`${q2}\``})
